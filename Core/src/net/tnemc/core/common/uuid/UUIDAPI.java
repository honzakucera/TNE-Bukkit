...
public interface UUIDAPI {
  ...
  default UUID getUUID(String username) {
    ...
    UUIDPair pair = (id != null)? new UUIDPair(id, username) : EconomyManager.connector()
                                                               .uuidProvider().generatePair(username);

    EconomyManager.connector().uuidProvider().store(pair);
    return id;
  }

  static String dashUUIDString(String uuid) {
    return uuid.replaceAll(EconomyManager.UUID_MATCHER_PATTERN.pattern(), "$1-$2-$3-$4-$5");
  }
